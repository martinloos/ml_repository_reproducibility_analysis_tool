...
def build_result(repository_url, output_file):
    ...
    if not readme_result:
        readme_result = build_zero_value_list(8)
    ...
    result = [repository_url, readme_result[0], readme_result[1], readme_result[2], readme_result[3], readme_result[4],
              readme_result[5], readme_result[6], readme_result[7], license_result[0], license_result[1], license_result[2],
...
def make_csv_header():
    return ['repository_url', '# readmes', 'total readme lines', 'average readme lines', '# readme links',
            '# readme paper links', '# not accessible links', '% not accessible links', 'readme dataset reference',
            '# licences', '# open-source licenses', '# non-open-source licenses', '# source code files',
            '# total lines of code', '% of code lines', '# total comment lines', '% of comment lines',
            'code-comment-ratio', 'average pylint score', '# jupyter notebooks', '% jupyter notebooks /w header',
...
def build_terminal_response():
    ...
    if analysis_result[8] == 0:
        table.add_row('readme dataset reference', 'No')
    else:
        table.add_row('readme dataset reference', 'Yes')
    table.add_row('# licenses', str(analysis_result[9]))
    table.add_row('# open-source licenses', str(analysis_result[10]))
    table.add_row('# non-open-source licenses', str(analysis_result[11]))
    table.add_row('# source code files', str(analysis_result[12]))
    table.add_row('# total lines of code', str(analysis_result[13]))
    table.add_row('% of code lines', str(analysis_result[14]))
    table.add_row('# total comment lines', str(analysis_result[15]))
    table.add_row('% of comment lines', str(analysis_result[16]))
    table.add_row('code-comment-ratio', str(analysis_result[17]))
    table.add_row('average pylint score (best is 10)', str(analysis_result[18]))
    table.add_row('# jupyter notebooks', str(analysis_result[19]))
    table.add_row('% of jupyter notebooks /w header', str(analysis_result[20]))
    table.add_row('% of jupyter notebooks /w footer', str(analysis_result[21]))
    table.add_row('# random seed lines', str(analysis_result[22]))
    table.add_row('% fixed random seed lines', str(analysis_result[23]))
    table.add_row('Model serializaton used?', str(analysis_result[24]))
    table.add_row('# hyperparameter logging indicators', str(analysis_result[25]))
    table.add_row('% public available libraries in relation to all unique imports in source code (excl. local libs '
                  'and python standard libs)', str(analysis_result[26]))
    table.add_row('# config files', str(analysis_result[27]))
    table.add_row('OS specified in config file(s)', str(analysis_result[28]))
    table.add_row('# unique imports in config file(s)', str(analysis_result[29]))
    table.add_row('# unique imports in source code file(s)', str(analysis_result[30]))
    ...
    table.add_row('% unique imports used in source code + mentioned in config file(s) in relation to all used',
                  str(analysis_result[33]))
    table.add_row('dataset folders found?', str(analysis_result[34]))
    table.add_row('# dataset folder candidates', str(analysis_result[35]))
    table.add_row('# dataset file candidates', str(analysis_result[36]))
    table.add_row('# found dataset file candidates mentioned in source code', str(analysis_result[37]))
    table.add_row('% mentioned dataset files in relation to all found candidate files',
                  str(analysis_result[38]))
    table.add_row('out-of-the-box buildable with binderhub?', str(analysis_result[39]))
    console.print(table)
    print('\n')
