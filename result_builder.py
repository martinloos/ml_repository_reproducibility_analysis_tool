...
def build_result(repository_url, output_file):
    ...
    if not source_code_result:
        source_code_result = build_zero_value_list(14)
...
def make_csv_header():
    return ['repository_url', '# readmes', 'total readme lines', 'average readme lines', '# readme links',
            ...
            'Data version control used?', '# hyperparameter documentation indicators', '# config files',
            'OS specified in config file(s)', '# unique imports in config file(s)',
...
def build_terminal_response():
    ...
    table.add_row('# unique imports in config file(s)', str(analysis_result[27]))
    table.add_row('# unique imports in source code file(s)', str(analysis_result[28]))
    table.add_row('% strict unique imports in relation to all unique imports in config file(s)',
                  str(analysis_result[29]))
    ...
    table.add_row('dataset folders found?', str(analysis_result[32]))
    table.add_row('# dataset folder candidates', str(analysis_result[33]))
    table.add_row('# dataset file candidates', str(analysis_result[34]))
