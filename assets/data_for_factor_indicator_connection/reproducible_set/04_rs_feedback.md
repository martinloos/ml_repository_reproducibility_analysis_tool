...
| ----------- | ----------- |
| Source-code availability and documentation |  0.97 |
| Software environment |  0.45 |
...
2. README overall scoring (score: 0.96 out of 1.0): See sub-ratings for more information.

- Average length scoring (score: 0.95 out of 1.0): We determined that 82 or more lines are best. We found 79.0 lines (in average) in the README file(s)
...
3. Source-code pylint scoring (score: 0.92 out of 1.0): Not normalized pylint rating is 5.24 where 10.0 is best. We found that readable code is best for reproducibility and consider ratings over 5.71 as desirable.

4. Source-code-comment-ratio scoring (score: 1.0 out of 1.0): We measured a average ratio of 1.59. We consider a ratio below 8.73 as best. But more comments are fine too (which would make the ratio smaller). Well documented code is important.
...
1. Libraries used in the source-code and mentioned in config file scoring (score: 0.27 out of 0.6): We found that 44.44% of the used libraries are defined in the config file(s). We expect that 100% of the in the source-code used relevant libraries are defined in the config file(s).

2. Strictly defined libraries in config file(s) scoring (score: 0.0 out of 0.2): We expect that 100% of the defined libraries in the config file(s) are strictly (==) defined. We found 4 libraries in the config file(s). From these 0.0% were strictly specified.
