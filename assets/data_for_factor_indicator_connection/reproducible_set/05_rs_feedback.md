...
| ----------- | ----------- |
| Source-code availability and documentation |  0.91 |
...
2. README overall scoring (score: 0.82 out of 1.0): See sub-ratings for more information.

- Average length scoring (score: 0.77 out of 1.0): We determined that 82 or more lines are best. We found 68.0 lines (in average) in the README file(s)
- Average accessible links scoring (score: 1.0 out of 1.0): We determined that 4 or more links are best. We found 13.499499999999998 accessible links (in average) in the README file(s)
3. Source-code pylint scoring (score: 0.98 out of 1.0): Not normalized pylint rating is 5.62 where 10.0 is best. We found that readable code is best for reproducibility and consider ratings over 5.71 as desirable.

4. Source-code-comment-ratio scoring (score: 1.0 out of 1.0): We measured a average ratio of 8.73. We consider a ratio below 8.73 as best. But more comments are fine too (which would make the ratio smaller). Well documented code is important.
...
1. Libraries used in the source-code and mentioned in config file scoring (score: 0.3 out of 0.6): We found that 50.0% of the used libraries are defined in the config file(s). We expect that 100% of the in the source-code used relevant libraries are defined in the config file(s).

2. Strictly defined libraries in config file(s) scoring (score: 0.08 out of 0.2): We expect that 100% of the defined libraries in the config file(s) are strictly (==) defined. We found 5 libraries in the config file(s). From these 40.0% were strictly specified.
...
Dataset availability and preprocessing scoring (score: 1.0 out of 1.0): Dataset reference was found in the README. This is not best practice, because one should always provide a dataset (if possible) locally in the repository in order for others to reproduce your repository with the same input data. Of course this is not always possible, especially when dealing with larger datasets.
...
Random seed lines with fixed seed scoring (score: 1.0 out of 1.0): We found that 100.0% of the 13 found random seed declaration lines had a fixed seed. If the value is not 100% make sure to fix the random seeds.
